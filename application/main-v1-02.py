...
class ConfigurationGUI(object):
    def __init__(self):
        ...
        # create main frames
        left_frame = tk.Frame(win, width=200, height=350, bg="blue")
        middle_frame = tk.Frame(win, width=280,height=350,bg="red")
        right_frame = tk.Frame(win, width=200, height=350, bg="silver")

        left_frame.grid(row=0, column=0)
        middle_frame.grid(row=0, column=1)
        right_frame.grid(row=0, column=2)

        # image = Image.open("diagram-v2.png")
        # background_image = ImageTk.PhotoImage(image)
        # background = tk.Label(middle_frame, image=background_image)
        # background.place(x=0, y=0)

        load = Image.open("diagram-v2.png")
        render = ImageTk.PhotoImage(load)
        img = tk.Label(middle_frame, image=render)
        img.image = render
        img.place(x=0, y=0)
